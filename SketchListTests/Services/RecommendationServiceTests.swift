...
// MARK: - getRecommendationsForSingleTrack(sourceKey:sourceBPM:candidates:limit:)

@MainActor
extension RecommendationServiceTests {
    ...
    func returnsRequestedNumberOfResults() throws {
        ...
        #expect(
            recommendations.count == recommendationLimit,
    ...
    func wrapsAroundWheel(source: CamelotKey, expected: CamelotKey) throws {
        ...
        #expect(
            recommendations.contains(where: { $0.key == expected }),
            "Source \(source.rawValue) should surface at least one \(expected.rawValue) candidate via wheel wrap"
        )
    }

    @Test(
        "Octave-equivalent tempos (half/double) count as compatible for the same key and its relative",
        arguments: [
    ...
    func octaveEquivalentTempo(sourceBPM: Double, matchBPM: Double, expectedKey: CamelotKey) throws {
        ...
        #expect(
            recommendations.contains(where: { $0.bpm == matchBPM && $0.key == expectedKey }),
            "Source 3B @ \(sourceBPM) should surface a \(expectedKey.rawValue) @ \(matchBPM) candidate (octave-equivalent tempo)"
        )
    }

    @Test(
        "Every recommendation is a valid Camelot transition (same key, ±1 on the wheel, or relative major/minor)",
    ...
    func recommendationsSortedByBpmScore() {
        ...
        #expect(
            recommendations.map(\.bpm) == [128, 132, 140, 152, 100, 168],
